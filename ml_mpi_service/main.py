...
MQ_SERVICE_URL = os.getenv("MQ_SERVICE_URL", "http://localhost:8003") # Your Deno service port
TRANSACTIONS_QUEUE_NAME = os.getenv("TRANSACTIONS_QUEUE_NAME", "transactions_queue")
...
FALLBACK_POLL_INTERVAL_SECONDS = float(os.getenv("FALLBACK_POLL_INTERVAL_SECONDS", "1.0"))

def load_model(filename=MODEL_PATH):
    if not os.path.exists(filename):
        # Critical error, inform all processes and abort
        print(f"[Process {RANK}] ERROR: Model file '{filename}' not found. Aborting MPI execution.")
        COMM.Abort(1) # Abort all MPI processes
        # The following line might not be reached if Abort is effective immediately
        raise FileNotFoundError(f'Model file with path "{filename}" not found.')
    try:
        model = joblib.load(filename)
        print(f"[Process {RANK}] Model '{filename}' loaded successfully.")
        return model
    except Exception as e:
        print(f"[Process {RANK}] ERROR: Failed to load model '{filename}': {e}. Aborting MPI execution.")
        COMM.Abort(1)
        raise
...
def fetch_batch_from_deno_mq(session, count):
    ...
    try:
        ...
        print(f"[MASTER {RANK}] Calling /pull-n: {pull_url} with timeout {HTTP_CLIENT_TIMEOUT}s")
        ...
        if response.status_code == 200:
            ...
            if isinstance(tasks, list):
                ...
                try:
                    ...
                    print(f"[MASTER {RANK}] Error decoding JSON in tasks: {e}. Tasks: {tasks}")
                    return []
                return tasks
            else:
                print(f"[MASTER {RANK}] Error: /pull-n did not return a list. Got: {type(tasks)}")
            ...
            print(f"[MASTER {RANK}] Error fetching batch from Deno MQ (/pull-n): {response.status_code} - {response.text}")
            return []
    except requests.exceptions.Timeout:
        print(f"[MASTER {RANK}] HTTP Request to /pull-n timed out after {HTTP_CLIENT_TIMEOUT}s.")
        return []
    except requests.exceptions.RequestException as e:
        print(f"[MASTER {RANK}] HTTP Request failed when pulling batch (/pull-n): {e}")
        return []
    except json.JSONDecodeError as e:
        print(f"[MASTER {RANK}] Failed to decode JSON response from Deno /pull-n: {e}")
        return []
...
def push_batch_to_deno_mq(session, results_batch):
    ...
    try:
        ...
        response = session.post(push_url, json=payload, timeout=HTTP_CLIENT_TIMEOUT) # Increased timeout for batch
        if response.status_code == 201:
            print(f"[MASTER {RANK}] Successfully pushed batch of {len(results_batch)} results.")
            return True
        else:
            print(f"[MASTER {RANK}] Error pushing batch to Deno MQ (/push-n): {response.status_code} - {response.text}")
            return False
    except requests.exceptions.RequestException as e:
        print(f"[MASTER {RANK}] HTTP Request failed when pushing batch (/push-n): {e}")
        return False

def run_master_node():
    num_workers = SIZE - 1
    print(f"[MASTER {RANK}] Initialized. Number of workers: {num_workers}. Using Deno MQ at {MQ_SERVICE_URL}")
    ...
    try:
        while True:
            ...
            tasks_to_process = fetch_batch_from_deno_mq(http_session, num_workers if num_workers > 0 else 1) # Fetch 1 if no workers (though master shouldn't run alone)


            if not tasks_to_process:
                # print(f"[MASTER {RANK}] No tasks fetched from /pull-n. Deno MQ might be empty or timed out (204).")
                # If Deno's long poll (30s) returns 204, we don't need to sleep aggressively here.
                # A short sleep can be a fallback if 204 is returned too quickly for some reason.
                time.sleep(FALLBACK_POLL_INTERVAL_SECONDS)
                continue

            num_actual_tasks = len(tasks_to_process)
            print(f"[MASTER {RANK}] Fetched {num_actual_tasks} tasks via /pull-n. Distributing to workers.")
            ...
            if num_workers > 0 and workers_to_use > 0:
                ...
                for i in range(workers_to_use):
                    worker_rank = active_worker_ranks[i]
                    try:
                        prediction_result = COMM.recv(source=worker_rank, tag=2)
                        results_from_workers.append(prediction_result)
                    except MPI.Exception as e:
                        print(f"[MASTER {RANK}] ERROR: MPI Exception while receiving from worker {worker_rank}: {e}")
                ...
                print(f"[MASTER {RANK}] Warning: Master node running without workers. This case should be handled by standalone mode.")
                # Process tasks directly if any were fetched (e.g. if num_workers was initially 1 then changed to 0)
                # This part is more of a fallback and ideally shouldn't be hit in normal operation.
                # For simplicity, we assume if num_workers is 0, this loop won't do MPI work.
                # The primary logic for SIZE=1 is in run_standalone_node.
                pass


            if results_from_workers:
                if push_batch_to_deno_mq(http_session, results_from_workers):
                    print(f"[MASTER {RANK}] Pushed batch of {len(results_from_workers)} results successfully via /push-n.")
                else:
                    print(f"[MASTER {RANK}] Failed to push batch of {len(results_from_workers)} results via /push-n.")
        ...
        if num_workers > 0:
            ...
            active_count_for_shutdown = workers_to_use if 'workers_to_use' in locals() and workers_to_use > 0 else num_workers
            print(f"[MASTER {RANK}] Sending STOP signal to up to {active_count_for_shutdown} workers.")
            for i in range(num_workers): # Iterate through all potential workers
                ...
                try:
                    ...
                    print(f"[MASTER {RANK}] Note: Failed to send STOP signal to worker {worker_rank} (may have already exited): {e}")
        http_session.close()
...
def preprocess_transaction_data(transaction_data: dict):
    ...
    """
    print(f"[Process {RANK}] Preprocessing transaction data: {transaction_data.get('id')}")
    try:
        # 1. Timestamp processing: Convert ISO string to Unix timestamp (float)
        # Example: "2023-01-01T12:00:00Z"
        dt_object = datetime.fromisoformat(transaction_data.get('timestamp', '').replace('Z', '+00:00'))
        ...
        status_numeric = status_mapping.get(transaction_data.get('status', '').lower(), -1) # Default to -1 if unknown
        if status_numeric == -1:
            print(f"[Process {RANK}] Warning: Unknown status '{transaction_data.get('status')}' for transaction {transaction_data.get('id')}. Using -1.")
        ...
        print(f"[Process {RANK}] Error preprocessing transaction {transaction_data.get('id', 'N/A')}: {e}")
        return None
...
def make_prediction(model, features_df: pd.DataFrame, transaction_id="N/A"):
    ...
    """
    print(f"[Process {RANK}] Making prediction for transaction: {transaction_id}")
    if model is None:
        print(f"[Process {RANK}] Error: Model not loaded for transaction {transaction_id}.")
        return {"transaction_id": transaction_id, "error": "Model not loaded", "is_fraudulent": -1, "confidence": 0.0}
    if not isinstance(features_df, pd.DataFrame):
        print(f"[Process {RANK}] Error: Features for transaction {transaction_id} are not a Pandas DataFrame.")
    ...
    try:
        ...
        print(f"[Process {RANK}] Prediction for {transaction_id}: Fraudulent={is_fraudulent}, Confidence={confidence:.4f}")
        return {"transaction_id": transaction_id, "timestamp": datetime.now().isoformat(), "is_fraudulent": is_fraudulent, "confidence": confidence}
    except Exception as e:
        print(f"[Process {RANK}] Error during prediction for transaction {transaction_id}: {e}")
    ...
    try:
        while True:
            ...
            if not features.empty:
                prediction_result = make_prediction(model, features, transaction_id)
            else:
                prediction_result = {
...
def run_standalone_node(model):
    ...
    print(f"[STANDALONE {RANK}] Running in single process mode. Using Deno MQ at {MQ_SERVICE_URL}")
    http_session = requests.Session()
    try:
        while True:
            ...
            transaction_id = task_data.get("id", "N/A")
            print(f"[STANDALONE {RANK}] Processing task for transaction: {transaction_id}")
            ...
            if push_batch_to_deno_mq(http_session, [prediction_result]): # Send as a list
                print(f"[STANDALONE {RANK}] Pushed result for transaction {transaction_id} via /push-n.")
            else:
                print(f"[STANDALONE {RANK}] Failed to push result for transaction {transaction_id} via /push-n.")
...
if __name__ == "__main__":
    ...
    if RANK > 0 or SIZE == 1: # Workers and standalone node load the model
        ...
        if not ml_model and SIZE > 1 : # If worker fails to load model
             print(f"[WORKER {RANK}] Failed to load model. Worker cannot proceed.")
    ...
    if SIZE == 1:
        ...
        if RANK == 0: # Master
            ...
            # else: worker already aborted if model load failed
